...
def _ensure_v3_feature_history_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in ["mature_reason", "reject_reason", "sample_quality_grade", "gate_version", "label_version", "_prob_src", "verify_date", "risk_level", "risk_label", "risk_tags", "intraday_status", "intraday_missing_reason"]:
        d[c] = d[c].astype(str)
    d["verify_date"] = d["verify_date"].map(_normalize_yyyymmdd_value)
